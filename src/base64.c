...
static const char b64c[65] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
void base64_encode(const char* in, size_t inlen, char* out, size_t outlen)
{
	...
	while(inlen && outlen)
	{
		...
		*out++ = b64c[((to_uchar(in[0]) << 4) + (--inlen ? to_uchar(in[1]) >> 4 : 0)) & 0x3f];
		if(!--outlen)
			break;
		
		*out++ = (inlen ? b64c[((to_uchar (in[1]) << 2) + (--inlen ? to_uchar(in[2]) >> 6 : 0)) & 0x3f] : '=');
		if(!--outlen)
			break;
